...
const JobOpportunities = () => {
  // Use 'typeof Slider' to correctly reference the type of the slider ref
  const scrollRef = useRef<typeof Slider | null>(null);
  ...
  const settings = {
    ...
    slidesToShow: 2.5,
    slidesToScroll: 1,
    nextArrow: <NextArrow onClick={() => scrollRef.current?.slickNext()} />,
    prevArrow: <PrevArrow onClick={() => scrollRef.current?.slickPrev()} />,
    arrows: false,
    responsive: [
      {
        breakpoint: 1024,
    ...
    <div className="lg:pl-32 px-12 pr-8 py-12 mx-auto">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-4">
          <PrevArrow onClick={() => scrollRef.current?.slickPrev()} />
          <NextArrow onClick={() => scrollRef.current?.slickNext()} />
        </div>
      </div>
          ...
          <Slider ref={scrollRef} {...settings} className="flex">
            ...
            <div className="bg-blue-200 text-black p-4 rounded-2xl w-52 space-y-4 border-2 border-white mr-4">
              ...
              <div className="flex text-xs justify-around gap-1 items-center">
                ...
                <span
                  className="text-blue-500 text-2xs font-semibold"
                  style={{ fontSize: "10px" }}
                >
                  4 hour ago
                </span>
              ...
              >
                <div className="flex text-xs gap-1 items-center">
                  <Image
                    src={Gitlab}
                    alt=""
                    className="p-1 rounded-lg bg-white w-10 h-10"
                  />
                  <div>
                    <p className="font-semibold">Gitlab</p>
                    <p className="flex gap-1">
                      <FaUserFriends /> 1200-3000
                    </p>
                  </div>
                </div>
